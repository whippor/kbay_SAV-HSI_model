...
kelpDF |>
  ...
  mutate(category = case_when(category == "HSI_value" ~ "canopy",
...
tamdata <- data.frame(continuous = c(10, 5, 3, 1),
                      value = c(0, 1, 1, 0))
...
tamdata2 <- data.frame(discrete = c("A", "B", "C", "D"),
...
ggplot() +
  ...
  scale_fill_viridis_c(option = "rocket", 
                       ...
                       direction = -1, 
                       na.value = "white") + # Use a viridis color scale
  theme_minimal() +
  guides(fill = "none") +
