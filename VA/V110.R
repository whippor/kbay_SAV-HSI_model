######################################################
### V110. Validation Sampling - Virtual - Seagrass ###
######################################################
...
### output directories
#### submodel directory
dir.create("data/e_validation_data/seagrass_HSI")
submodel_dir <- "data/e_validation_data/seagrass_HSI"
...
bathmask <- clamp(bathymetry, lower = -10 - MLLWcor, upper = 3 - MLLWcor, values = FALSE)
...
# with bathmask
model_diff10 <- mask(model_diff, bathmask)
model_diff10 <- model_diff10[["lyr1"]]
plot(model_diff10, col = viridis(nrow(model_diff), option = "turbo"))

# histogram of performance values with 10 m cutoff
 seagrass_perf_df <- data.frame(model_diff10)
 seagrass_perf_df %>%
ggplot(aes(x=lyr1)) +
